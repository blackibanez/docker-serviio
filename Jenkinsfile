      ...
      steps {
           withCredentials([sshUserPrivateKey(credentialsId: "private_key", keyFileVariable: 'keyfile', usernameVariable: 'NUSER')]) 
           {
            ...
            sh '''
              ...
              ssh -o StrictHostKeyChecking=no -i ${keyfile} -p ${PORT_SSH} ${NUSER}@${PRODUCTION_IP_HOST} docker stop $IMAGE_NAME  || true
              ssh -o StrictHostKeyChecking=no -i ${keyfile} -p ${PORT_SSH} ${NUSER}@${PRODUCTION_IP_HOST} docker rm $IMAGE_NAME  || true
              ...
              ssh -o StrictHostKeyChecking=no -i ${keyfile} -p ${PORT_SSH} ${NUSER}@${PRODUCTION_IP_HOST} docker run --name $IMAGE_NAME -d -p 23423:23423 -p 8895:8895 -p 1900:1900 -v $SERVIIO_VOLUME:/opt/serviio -v $MEDIA_VOLUME:/media/serviio/adult -v $DOWNLOADS_VOLUME:/media/serviio/downloads  blackibanez/$IMAGE_NAME:$IMAGE_TAG  || true
            '''
             }
           }
        }
